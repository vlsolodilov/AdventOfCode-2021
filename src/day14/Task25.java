...
public class Task25 {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<String> newPolymer = new ArrayList<>(polymer);
            for (int i = 0; i < 10; i++) {
                for (int j = 0; j < polymer.size()-1; j++) {
                    newPolymer.add(2 * j + 1, pairs.get(polymer.get(j) + polymer.get(j+1)));
                }
                polymer = new ArrayList<>(newPolymer);
            }

            Map<String, Integer> letterCount = new HashMap<>();
            for (int i = 0; i < polymer.size(); i++) {
                if (!letterCount.containsKey(polymer.get(i)))
                    letterCount.put(polymer.get(i), 1);
                else
                    letterCount.merge(polymer.get(i), 1, Integer::sum);
            }
